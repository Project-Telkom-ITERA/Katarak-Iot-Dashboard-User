...
import {
  ...
  ImageOff, Zap, Loader, User, Save, Lock, LogOut, ShieldCheck
} from "lucide-react";
    ...
    <div className="min-h-screen bg-[#0b1120] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-[#0d2e24] border border-[#1a5c42] rounded-2xl flex items-center justify-center mx-auto mb-4">
            <ShieldCheck size={28} className="text-[#34d399]" />
          </div>
          <p className="text-[11px] text-[#34d399] tracking-widest font-bold mb-1">ADMIN DASHBOARD</p>
          <h1 className="text-[20px] text-gray-100 font-semibold">Katarak IoT Detector</h1>
          <p className="text-[13px] text-gray-500 mt-1">Masuk untuk mengelola data pasien</p>
        </div>

        <div className="bg-[#1a2332] border border-[#243044] rounded-2xl p-6 flex flex-col gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-[#111a27] border border-[#243044] rounded-xl pl-9 pr-4 py-3 text-[13px] text-gray-200 outline-none focus:border-[#34d399] placeholder-gray-600"
              />
            </div>
          </div>
          <div>
          ...
          {err && (
            <div className="bg-red-900/30 border border-red-800 rounded-xl px-3 py-2 text-[12px] text-red-400 flex items-center gap-2">
              <AlertCircle size={13} /> {err}
            </div>
          )}
          <button onClick={handleLogin} disabled={loading}
            className="w-full bg-[#0d2e24] hover:bg-[#1a5c42] border border-[#1a5c42] rounded-xl py-3 text-[13px] text-[#34d399] font-semibold flex items-center justify-center gap-2 transition-colors disabled:opacity-60">
            {loading ? <div className="w-4 h-4 border-2 border-[#34d399] border-t-transparent rounded-full animate-spin" /> : <ShieldCheck size={15} />}
            {loading ? "Memverifikasi..." : "Masuk"}
          </button>
        </div>
        <p className="text-center text-[11px] text-gray-600 mt-4">Hanya untuk operator yang berwenang</p>
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  const fetchLatestCapture = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        // Reset form jika foto baru berbeda
    ...
    <div className="bg-[#1a2332] rounded-2xl p-4 border border-[#243044]">
      <div className="flex items-center gap-2 mb-3">
        <div className="bg-[#0d2e24] px-3 py-1 rounded-full flex items-center gap-1.5 border border-[#1a5c42]">
          ...
          <span className="text-[11px] text-[#34d399] tracking-widest font-bold">ADMIN — KATARAK IoT</span>
        </div>
        <button onClick={() => { sessionStorage.removeItem("admin_auth"); setIsLoggedIn(false); }}
          className="ml-auto flex items-center gap-1 text-[11px] text-gray-600 hover:text-gray-400 transition-colors">
          <LogOut size={12} /> Keluar
        </button>
      </div>
      <div className="flex items-start gap-3 mb-3">
        <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0 border border-[#243044]">
        ...
        <div>
          ...
          <p className="text-[12px] text-gray-500 mt-0.5">Berbasis IoT — ESP32-CAM · Dashboard Admin</p>
        </div>
      </div>
  ...
  const DesktopNav = () => (
    <>
      <div className="bg-[#0f1929] border-b border-[#1e2d40] px-8 py-3 flex items-center gap-4">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg overflow-hidden border border-[#243044]"><img src={cataractImg} alt="Katarak" className="w-full h-full object-cover" /></div>
          <div className="bg-[#0d2e24] px-3 py-1 rounded-full flex items-center gap-1.5 border border-[#1a5c42]">
            <Eye size={13} className="text-[#34d399]" />
            <span className="text-[11px] text-[#34d399] tracking-widest font-bold">ADMIN — KATARAK IoT DETECTOR</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { sessionStorage.removeItem("admin_auth"); setIsLoggedIn(false); }}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#243044] text-[12px] text-gray-500 hover:text-gray-300 hover:bg-[#1a2332] transition-colors">
      ...
      <div className="lg:hidden flex justify-center">
        <div className="w-full max-w-md min-h-screen flex flex-col pb-6 px-3 pt-4 gap-3">
          ...
          <div className="mt-2 flex items-center justify-center gap-1.5">
            ...
            <p className="text-[11px] text-gray-600">Deteksi Katarak IoT — Admin · 2026</p>
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:flex flex-col min-h-screen">
        ...
        <div className="flex-1 px-8 py-6 overflow-auto">
          ...
          <div className="mt-6 flex items-center justify-center gap-1.5 max-w-6xl mx-auto">
            <Eye size={12} className="text-[#34d399]" />
            <p className="text-[11px] text-gray-600">Deteksi Katarak IoT — Admin Dashboard · 2026</p>
          </div>
        </div>
      </div>
    </div>
